...
with open('fulldata20.csv', 'w', newline='', encoding='utf-8') as csvfile:
    writer = csv.writer(csvfile)

    # Write header row
    writer.writerow(['Link', 'Title', 'Rating', 'Price', 'No. of Reviews', 'Description', 'Manufacturer'])
    ...
    for div in div_elements:
        ...
        # Find div element for description
        div_description = div.find('div', class_='a-section a-spacing-medium a-spacing-top-small')
        description = div_description.text if div_description else ''

        # Find a element for manufacturer
        a_manufacturer = div.find('a', class_='a-link-normal')
        manufacturer = a_manufacturer.text if a_manufacturer else ''

        # Write the data to the CSV file
        writer.writerow([link, title, rating, price, no_reviews, description, manufacturer])
